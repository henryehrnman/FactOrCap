(() => {
  ...
  async function checkOneAndApply(scanId, claim, mode) {
    ...
    if (response?.error) {
      ...
      claim.source = r.source || (mode === 'enhanced' ? 'enhanced' : 'fact-checker');
  ...
  const STRIP_SELECTORS = [
    ...
    'form', 'fieldset', 'select', 'menu', 'button', 'textarea', 'input',
    'figcaption',
    '[aria-hidden="true"]',
    '[role="navigation"]', '[role="banner"]', '[role="contentinfo"]',
    '[role="complementary"]', '[role="search"]', '[role="dialog"]',
    '[role="note"]', '[role="menubar"]', '[role="tablist"]',
    ...
    '.reference', '.reflist', '.references',
    '.navbox', '.navbox-inner', '.toc', '.toctitle',
    '.hatnote', '.metadata', '.shortdescription',
    '.vector-page-tools', '.vector-menu', '.vector-appearance-landmark',
    // Generic banner / cookie / consent UI
    '.cookie-banner', '[id*="cookie-banner"]', '[id*="consent-banner"]'
  ].join(', ');
  ...
  function extractClaims(text) {
    ...
    return sentences
      ...
      .filter((s) => verbPattern.test(s))
      .filter((s) => !UI_NOISE_PATTERN.test(s))
      .slice(0, 15);
  }

...
Fact-checking Chrome extension. Scans the page you're on, extracts checkable claims, and tells you what's **fact** and what's **cap**.

Powered by the [Google Fact Check Tools API](https://developers.google.com/fact-check/tools/api). Claims that aren't in the fact-check index are enriched with the matching [Wikipedia](https://en.wikipedia.org/) summary so you can read context inline instead of getting a dead "Unverified" card.
...
3. Create your local config:
   ...
   Open `config.js` and paste your Fact Check Tools key in place of `__GOOGLE_FACT_CHECK_API_KEY__`. `config.js` is git-ignored.
...
4. If no published fact-check exists for a claim, the extension queries the public Wikipedia REST API for the most relevant article and attaches its summary + URL as context.
5. Claims are highlighted inline and listed in a sidebar, color-coded by verdict:
   ...
   - **Unverified** — no matching fact-check found. When Wikipedia has a relevant article, the card includes its summary and a clickable link to the full page.
...
3. **Restrict the key in Google Cloud** so a leaked-from-zip key can't be reused for anything else: [console.cloud.google.com](https://console.cloud.google.com/) → Credentials → click the key → **API restrictions → Restrict key → Fact Check Tools API**.
4. (Optional) Set a per-day quota cap on the API to prevent runaway use.

## Security note

The pre-built release zip contains the Fact Check Tools API key in plain JavaScript. Anyone who downloads and unzips it can read it. This is acceptable for FactOrCap because:

- The key is restricted to a single, low-stakes API (Fact Check Tools).
- The API has a generous free tier and shared use is the intent.
- A misbehaving user can only burn shared quota, not run up a bill — assuming you don't enable billing on the project.

The Wikipedia REST API used for the no-fact-check fallback is free, anonymous, and requires no key.

If you ever need stronger isolation, switch to a settings-page model where each user pastes their own key into the extension (stored in `chrome.storage.sync`) instead of shipping a baked-in key.
